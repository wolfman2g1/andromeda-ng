...
from andromeda_ng.service.crud import lead_crud
...
async def create_lead(lead_data: LeadSchema, db=Depends(get_db)):
    ...
    lead_data.lead_email = lead_data.lead_email.lower()  # Normalize email to lowercase for case-insensitive search
    # Check if lead already exists
    check_lead = await lead_crud.read_lead_by_email(db, lead_data.lead_email)
    if check_lead:
        logger.error(f"Lead already exists with email: {lead_data.lead_email}")
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Lead already exists")
    lead = await lead_crud.create_lead(db, lead_data)
    if not lead:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Please check your input data")
    return lead

@router.get("/",response_model=LeadOutput,  status_code=status.HTTP_200_OK)
async def read_leads(db=Depends(get_db)):
    try: 
        logger.info("Reading leads")
        leads = await lead_crud.read_leads(db)
        return leads
...
async def read_lead_by_id(lead_id: uuid.UUID, db=Depends(get_db)):
    try:
        lead = await lead_crud.read_lead_by_id(db, lead_id)
...
async def update_lead(lead_id: uuid.UUID, lead_data: LeadSchema, db=Depends(get_db)):
    try:
        check_lead = await lead_crud.read_lead_by_id(db, lead_id)
        if not check_lead:
            logger.error(f"Lead not found with id: {lead_id}")
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Lead not found")
        updated_lead = await lead_crud.update_lead(db, lead_id, lead_data)
        if not updated_lead:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Please check your input data")
...
async def delete_lead(lead_id: uuid.UUID, db=Depends(get_db)):
    try:
        ...
        if not check_lead:
            logger.error(f"Lead not found with id: {lead_id}")
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Lead not found")
        await lead_crud.delete_lead(db, lead_id)
